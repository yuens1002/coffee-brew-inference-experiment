...
| `recommend` | Community-consensus brew recommendation (origin + roast + variety → params + confidence). Returns brew parameters (temp, ratio, grind, time), confidence tier, sources, method-specific technique guidance (bloom timing, pour stages, steep times), and community vote counts. |
